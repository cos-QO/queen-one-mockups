...
| CreativeSphere — Asset Library | [Open preview](https://cos-qo.github.io/queen-one-mockups/wireframes/creativesphere-asset-library.html) | `wireframes/creativesphere-asset-library.html` |
...
Click any **Open preview** link above. It opens via [htmlpreview.github.io](https://htmlpreview.github.io), which renders the raw HTML directly — so no download needed.
...
- **Color palette**: grayscale only — so no brand colors, no hex values outside the approved set
- **Elevation**: borders (`#E0E0E0`) only — so no shadows, no MUI `elevation` props
- **Typography**: Inter (Google Fonts CDN)
- **Interactivity**: all screens, modals, drawers, so and overlays are wired — nothing is static
